...
const getCommandIfSupported = command =>
  find(propEq("name", command))(supportedCommands);

const getFlags = words => {
  let flags = [];

  map(word => {
    if (length(match(/-/, word)) > 0) flags = [...flags, word];
  }, words);
  return flags;
};
...
export const getCommand = (line, termId) => {
  ...
  if (length(words[0]) === 0) return [];
  if (!isNil(command)) {
    const { name, action, supportedFlags } = command;
    const arg = length(words) > 1 ? takeLast(1, words)[0] : "";

    store.dispatch(addToCommandsHistory(termId, line));
    if (name === "ls") ret = action(termId);
