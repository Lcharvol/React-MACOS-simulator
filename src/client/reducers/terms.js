...
const getNewPath = (oldPath, dest, tree) => {
  if (!isDestAvailable(oldPath, tree)) return oldPath;
  if (dest == "..") return length(oldPath) > 1 ? dropLast(1, oldPath) : oldPath;
  return [...oldPath, dest];
};
...
const reducer = (state = initialState, action) => {
  switch (action.type) {
    case CHANGE_LOCATION: {
      ...
      let path = getNewPath(
        state[termIndex].path,
        action.dest,
        state[termIndex].tree
      );
    ...
    case ADD_TO_COMMAND_HISTORY: {
      const termIndex = findIndex(propEq("id", action.termId))(state);
      state[termIndex].history.commands = [
        ...state[termIndex].history.commands,
        action.command
      ];
      if (length(state[termIndex].history.commands) > 5)
        state[termIndex].history.commands = drop(
          1,
          state[termIndex].history.commands
        );
      return [...state];
    ...
    case ADD_NEW_LINE: {
      ...
      if (action.ret === null) return [...state];
      state[termIndex] = {
        ...state[termIndex],
        lines: [
          ...state[termIndex].lines,
          {
            ...
            values: !isNil(action.ret)
              ? [action.line, ...action.ret]
              : [action.line]
          }
        ]
      };
